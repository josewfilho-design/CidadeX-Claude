...
import { Tag, Plus, X, Pencil, Check, Eye, EyeOff, ArrowDownAZ, ArrowUpZA } from "lucide-react";
...
const CategoriesManager = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    const [customRes, renameRes, inactiveRes] = await Promise.all([
      ...
      supabase.from("user_custom_options").select("id, value").eq("user_id", user.id).eq("option_type", "cat_rename"),
  ...
  const toggleActive = async (itemKey: string) => {
    ...
    if (isCurrentlyInactive) {
      await supabase.from("user_custom_options").delete().eq("user_id", user.id).eq("option_type", "cat_inactive").eq("value", itemKey);
      setInactiveItems(prev => { const s = new Set(prev); s.delete(itemKey); return s; });
      toast.success("Ativada!");
    } else {
      await supabase.from("user_custom_options").insert({ user_id: user.id, option_type: "cat_inactive", value: itemKey });
  ...
  const handleAdd = async () => {
    ...
    const allNames = [...DEFAULT_CATEGORIES.map(c => renames[c] || c), ...customCategories];
    if (allNames.some(c => c.toLowerCase() === value.toLowerCase())) { toast.error("Essa categoria já existe."); return; }
    const { error } = await supabase.from("user_custom_options").insert({ user_id: user.id, option_type: "finance_category", value });
    if (error) { toast.error("Erro ao adicionar."); }
    else { toast.success(`"${value}" adicionada!`); setCustomCategories(prev => [...prev, value].sort()); setNewCategory(""); }
  };
  ...
  const handleRenameDefault = async (original: string) => {
    ...
    const allNames = [...DEFAULT_CATEGORIES.map(c => c === original ? null : (renames[c] || c)).filter(Boolean), ...customCategories];
    if (allNames.some(c => c!.toLowerCase() === newName.toLowerCase())) { toast.error("Esse nome já existe."); return; }
    const { data: existing } = await supabase.from("user_custom_options").select("id").eq("user_id", user.id).eq("option_type", "cat_rename").like("value", `%"from":"${original}"%`).maybeSingle();
    ...
    if (existing) {
      ...
      const { error } = await supabase.from("user_custom_options").insert({ user_id: user.id, option_type: "cat_rename", value: jsonValue });
      if (error) { toast.error("Erro ao renomear."); return; }
    }
    toast.success(`Renomeada para "${newName}".`);
  ...
  const handleEditCustom = async (oldValue: string) => {
    ...
    const allNames = [...DEFAULT_CATEGORIES.map(c => renames[c] || c), ...customCategories.filter(c => c !== oldValue)];
    if (allNames.some(c => c.toLowerCase() === value.toLowerCase())) { toast.error("Esse nome já existe."); return; }
    const { error } = await supabase.from("user_custom_options").update({ value }).eq("user_id", user.id).eq("option_type", "finance_category").eq("value", oldValue);
    if (error) { toast.error("Erro ao renomear."); }
    else { toast.success(`Renomeada para "${value}".`); setCustomCategories(prev => prev.map(c => c === oldValue ? value : c).sort()); setEditingItem(null); }
  };

  const handleRemove = async (category: string) => {
    if (!user) return;
    const { error } = await supabase.from("user_custom_options").delete().eq("user_id", user.id).eq("option_type", "finance_category").eq("value", category);
    if (error) { toast.error("Erro ao remover."); }
    else { toast.success(`"${category}" removida.`); setCustomCategories(prev => prev.filter(c => c !== category)); }
  };
  ...
  const allItems = sortItems([
    ...DEFAULT_CATEGORIES.map(c => ({ key: c, display: renames[c] || c, isDefault: true })),
    ...customCategories.map(c => ({ key: c, display: c, isDefault: false })),
  ]);
  ...
  return (
    <div className="glass-card rounded-xl p-4 space-y-3">
      ...
      <p className="text-xs text-muted-foreground">
        Gerencie suas categorias financeiras. Use 👁 para ativar/desativar.
      </p>
      ...
      <div className="flex gap-2">
        <input type="text" value={newCategory} onChange={e => setNewCategory(e.target.value)}
          ...
          placeholder="Nova categoria..." maxLength={50}
        ...
        <button onClick={handleAdd} disabled={!newCategory.trim()} title="Adicionar categoria"
